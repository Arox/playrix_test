    ...
    try:
        ...
        for data in filter_installs:
            if data.country not in total_result:
                total_result[data.country] = {INSTALLS_NAME: 0, **template_row}
            total_result[data.country][INSTALLS_NAME] += 1
        ...
        for data in filter_purchases:
            ...
            while index <= rpi_count:
                total_result[data.country][index] += data.revenue
                index += 1

        for country in total_result.keys():
            result.append([country, total_result[country][INSTALLS_NAME]] +
                          [total_result[country][index] / total_result[country][INSTALLS_NAME] for index in range(1, rpi_count+1)])
...
def main(start_date, end_date, app_id):
    ...
    filter_installs = filter(lambda x: start_date <= x.created < end_date,
                             read_csv_table('./installs.csv', InstallsObject, filter_for_csv))
    # итератор данных purchases
    filter_purchases = filter(lambda x: start_date <= x.install_date < end_date,
                              read_csv_table('./purchases.csv', PurchasesObject, filter_for_csv))
    # запуск вычислений
